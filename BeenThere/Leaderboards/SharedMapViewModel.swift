...
class SharedMapViewModel: NSObject, ObservableObject {
    ...
    func addSquaresToMap(locations: [Location]) {
        ...
        let setupLayers = { [weak self] in
            do {
                ...
                fillLayer.fillColor = .constant(StyleColor(self!.isDarkModeEnabled ? UIColor.green : UIColor.green))
...
//            self?.adjustMapViewToFitSquares()
    ...
    func centerMapOnLocation(location: CLLocation) {
        ...
        let zoomLevel = 2 // Adjust the zoom level as needed
